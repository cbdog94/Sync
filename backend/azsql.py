...
class AzureSqlClient:
    ...
    def __get_conn(self):
        ...
        conn = pyodbc.connect(self.conn_str, attrs_before={SQL_COPT_SS_ACCESS_TOKEN: token_struct})
        return conn
    ...
    def get(self, key):
        with self.__get_conn() as conn:
            ...
            if row:
                value, expiry = row
                if expiry and datetime.datetime.now(datetime.timezone.utc) > expiry.replace(tzinfo=datetime.timezone.utc):
                    self.delete(key)
                    return None
                return value
            return None
    ...
    def set(self, key, value, expiry_seconds=3600):
        with self.__get_conn() as conn:
            cursor = conn.cursor()
            expiry = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(seconds=expiry_seconds)
            cursor.execute("""
